...
import { Specialty, SpecialtyRequest } from '../../shared/models/specialty.model';
...
export class AdminSpecialtyManagementPage implements OnInit {
  ...
  submit(): void {
    ...
    if (this.specialtyForm.invalid || this.isSaving()) {
      return;
    }
  ...
  private getErrorMessage(error: unknown, fallback: string): string {
    if (typeof error === 'object' && error !== null && 'status' in error) {
      if (error.status === 403) {
        return 'You do not have permission to manage specialties.';
      }

      if (error.status === 409) {
        return 'A specialty with this name already exists.';
      }
    }
